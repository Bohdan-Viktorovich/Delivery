...
# Загружаем переменные окружения из .env (локально) или из Railway Variables
load_dotenv()
...
# --- Многоязычные тексты (как в исходном коде) ---
LANGUAGES = {
    'ru': {
...
# Хранилище языка пользователя (в оперативной памяти, при перезапуске сбрасывается)
user_lang = {}
...
async def process_cargo_details(message: types.Message, state: FSMContext):
    ...
    # Расчёт стоимости (здесь можно усложнить логику)
